    ...
    public function index(Request $request)
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'sales_commission' => 'required|integer',
    ...
    {
        return response()->json(crm_sales::findOrFail($id));
    }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            Log::info('Sale Updated', ['sale_id' => $crmSale->id, 'company_id' => $companyId, 'user_id' => $userId]);
    
            return response()->json(['success' => true, 'message' => 'Sale updated successfully', 'data' => $crmSale], 200);
        } catch (\Exception $e) {
    ...
    public function export(Request $request)
    {
        try {
            ...
            $query = crm_sales::where('company_id', $companyId)->where('user_id', $userId);
            ...
            // Check export format
            $exportFormat = strtolower($request->export_format ?? 'csv');
            $fileName = 'sales_data_' . date('Y-m-d_H-i-s');
    
            if ($exportFormat === 'csv') {
                // Generate CSV content
                $csvData = "Deal Name,Deal Value,Stage,Expected Close Date,Probability,Description,Created At\n";
    
                foreach ($salesData as $sale) {
                    $csvData .= "\"{$sale->deal_name}\",{$sale->deal_value},\"{$sale->stage}\","
                             . "{$sale->expected_close_date},{$sale->probability},\"{$sale->description}\","
                             . "{$sale->created_at}\n";
                }
                ...
                // Add Headers
                $headers = ["Deal Name", "Deal Value", "Stage", "Expected Close Date", "Probability", "Description", "Created At"];
            ...
            Log::error('Error exporting sales data', ['error' => $e->getMessage()]);
            return response()->json(['success' => false, 'error' => 'Internal Server Error'], 500);
        }
    }
...
public function getDashboardStats(Request $request)
{
    try {
        ...
        $wonDealsCount = (clone $baseQuery)->where('stage', 'won')->count();
