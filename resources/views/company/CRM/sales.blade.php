                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="deal-status-card">
                            ...
                            <small class="text-muted">65% to goal</small>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="timeline">
                        <div class="timeline-item">
                            ...
                            <div class="timeline-content">
                                ...
                                <p>First meeting with client to discuss requirements</p>
                            </div>
                        </div>
                        <div class="timeline-item">
                            <div class="timeline-date">2025-01-30</div>
                            <div class="timeline-content">
                                <h6>Proposal Sent</h6>
                                <p>Detailed proposal and pricing sent to client</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        success: function(response) {
            ...
            response.data.forEach(deal => {
                tableBody += `<tr>
                    ...
                    <td><span class="badge bg-success-subtle text-success">${deal.status}</span></td>
                    <td>
...
$(document).on("click", ".viewDeal", function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#viewdealCompany").text(response.deal_company); 
            $("#viewdealValue").text(`$${response.deal_value}`); 
            $("#viewdealStage").text(response.stage).removeClass().addClass("badge bg-info"); 
            $("#viewdealProbability").text(response.probability ? response.probability + "%" : "N/A"); 
            $("#viewdealCloseDate").text(response.expected_close_date); 
            $("#viewdealDescription").text(response.description); 
...
$(document).on("click", ".editDeal", function(e) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#edit_deal_id").val(response.id);
            $("#edit_deal_name").val(response.deal_name);
            $("#edit_company_id").val(response.company_id);
            $("#edit_deal_value").val(response.deal_value);
            $("#edit_stage").val(response.stage);
            $("#edit_expected_close_date").val(response.expected_close_date);
            $("#edit_probability").val(response.probability);
