<!-- Gmail-like Deleted Emails List -->
<style>
    .emails-container {
        background: #fff;
        border-radius: 16px;
...
<div class="emails-container">
    ...
    <!--
        <div class="email-header">
            ...
            <div class="recipient-cell">Recipient</div>
        ...
        @foreach($deletedEmails as $email)
            <div class="email-item">
                ...
                <div class="recipient-cell">{{ $email->recipient }}</div>
                ...
                <div class="time-cell">{{ $email->deleted_at->format('M d') }}</div>
                <div class="actions-cell">
                    <button class="btn btn-sm text-primary" title="View Email" onclick="$('#viewEmailModal{{ $email->id }}').modal('show')">
                        <i class="fas fa-eye"></i>
                    </button>
                    <button class="btn btn-sm text-success" title="Restore" onclick="restoreEmail({{ $email->id }})">
                        <i class="fas fa-undo"></i>
                    </button>
                    <button class="btn btn-sm text-danger" title="Delete Permanently" onclick="permanentlyDeleteEmail({{ $email->id }})">
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content email-modal-content">
                        ...
                        <div class="modal-body email-modal-body">
                            ...
                            <div class="email-info-group">
                                <div class="email-label">Deleted At:</div>
                                <div class="email-value">{{ $email->deleted_at->format('F j, Y g:i A') }}</div>
                            </div>
                        </div>
                        <div class="modal-footer email-modal-footer">
                            ...
                            <button type="button" class="btn btn-modal btn-modal-restore" onclick="restoreEmail({{ $email->id }})">
                                <i class="fas fa-undo"></i>Restore Email
                            </button>
                            <button type="button" class="btn btn-modal btn-modal-delete" onclick="permanentlyDeleteEmail({{ $email->id }})">
...
function restoreEmail(emailId) {
    ...
    Swal.fire({
        title: 'Restore Email?',
        text: "This email will be moved back to sent items",
        icon: 'question',
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '{{ route("company.email.restore") }}',
                method: 'POST',
                data: {
                    email_id: emailId,
                    _token: '{{ csrf_token() }}'
                ...
                success: function(response) {
                    if (response.success) {
                        // Update navigation to Sent
                        $('.nav-item').removeClass('active');
                        $('.nav-link').filter(function() {
                            return $(this).find('i.fa-paper-plane').length > 0;
...
function permanentlyDeleteEmail(emailId) {
    Swal.fire({
        title: 'Are you sure?',
        text: "This email will be permanently deleted and cannot be recovered!",
        icon: 'warning',
        showCancelButton: true,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('.emails-container').html(response.html);
