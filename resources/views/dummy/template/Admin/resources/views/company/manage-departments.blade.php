...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-centered table-striped" id="departmentsTable">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @forelse($departments ?? [] as $department)
                                        <tr>
                                            ...
                                            <td>{{ $department->created_at ? $department->created_at->format('M d, Y') : 'N/A' }}</td>
                                            <td class="text-center">
                                                ...
                                                <form action="{{ route('company.departments.toggle-status', $department->id) }}" 
                                                      method="POST" 
                                                      class="d-inline">
                                                    @csrf
                                                    <button type="submit" 
                                                            class="btn btn-sm {{ $department->status ? 'btn-warning' : 'btn-success' }} action-btn me-1"
                                                            title="{{ $department->status ? 'Deactivate' : 'Activate' }}">
                                                        <i class="fa-solid {{ $department->status ? 'fa-ban' : 'fa-check' }}"></i>
                                                    </button>
                                                </form>

                                                <form action="{{ route('company.departments.destroy', $department->id) }}" 
                                                      method="POST" 
                                                      class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                                                            ...
                                                            title="Delete"
                                                            onclick="return confirmDelete(event, '{{ $department->name }}')">
    ...
    <!-- Edit Department   NOTE: ACTION TO ROUTE AND STORE IS NOT SET YET. HSW 03/01/25-->
...
@section('script')
    ...
    <script>
        ...
        function editDepartment(id, name, description) {
            // Set form action
            document.getElementById('editDepartmentForm').action = `/company/departments/${id}`;
